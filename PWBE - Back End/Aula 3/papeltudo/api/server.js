...
//Rota de teste
const teste = (req, res) => {
    res.send("Back-end da papelaria respondendo");
}
...
const create = (req, res) => {
    ...
    let query = `INSERT INTO clientes(id, nome, descrição, valor) VALUE`;
...
const update = (req, res) => {
    ...
    let nome = req.body.nome;
    let sobrenome = req.body.descrição;
    let nascimento = req.body.valor;
    let query = `UPDATE clientes SET id = '${id}', nome = '${nome}', descrição = '${descrição}', valor = '${valor}' WHERE id = ${id};`;
    con.query(query, (err, result) => {
        if (err)
